...
- Combined language/image DAT: the successful bidirectional Resource Flash
  ...
  matches the DAT.
- Resource Flash start command `E3`: seven observations and static analysis
...
official-CPS discovery capture is required for this evidence set. Browser
support remains gated by implementation and one physical validation of each
enabled package kind.
...
pinned by the verifier. This explains the six distinct accepted payloads
...
Completed discovery captures do not need repeating. These distinct product
release gates remain:

1. Run the browser implementation on a physical Radio and verify final installed
   versions plus a normal full Codeplug read.
2. Test a second compatible Radio/hardware instance.
3. Establish non-destructive recovery behavior for each interruption phase.

Until then, implementation may proceed behind fixtures, but destructive update
actions must remain capability-gated and clearly labeled as not yet physically
